...
(function () {
  ...
  eq(st.A.cols.free.final, 220, "A final free = suma Δ = 100 + 120 = 220 (bez bazy)");
  eq(st.A.total, 220, "A total = 220 (tylko suma Δ)");
  eq(st.B.total, -90, "B total = −100 + 10 = −90 (tylko Δ)");
  eq(st.C.total, -130, "C total = −120 − 10 = −130 (tylko Δ)");
  eq(st.A.total + st.B.total + st.C.total, 0, "zero-sum: suma totali wszystkich graczy = 0");
